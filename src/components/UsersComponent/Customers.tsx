...
export default function Customers() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const customers = (response.data.users || response.data || []).filter(
  ...
  const updateUser = async (
    ...
    try {
      console.log("🔄 Updating Customer:", userId, userData);
  ...
  const deleteUser = async (userId: string | number) => {
    try {
      console.log("🗑️ Deleting Customer:", userId);
  ...
  const handleSuspendUser = async (user: User) => {
    try {
      ...
      console.log(
        `🔄 ${isSuspended ? "Activating" : "Suspending"} Customer:`,
        userId,
      ...
      if (isSuspended) {
        ...
        console.log("✅ Customer activated successfully:", response.data);
        message.success("Customer activated successfully");
      } else {
        response = await axios.patch(
          apiUrl(`${API_CONFIG.ENDPOINTS.AUTH.SuspendUser}${userId}`)
        );
        console.log("✅ Customer suspended successfully:", response.data);
  ...
  return (
    <div>
      <PageMeta
        title="AY Developers | Customers"
        description="Manage Customers and track their performance"
      />
        ...
        <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 text-[12px] md:text-sm">
              Manage Customers, track performance, and handle user operations
            </p>
          </div>
